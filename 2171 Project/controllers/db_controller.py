...
class DbController:
    ...
    def check_membership(self, club_id, student_id,thename):
        query2 = "SELECT * FROM student where id = %s and name = %s"
        self.cursor.execute(query2, (student_id, thename))
        val = self.cursor.fetchone()
        if val:
            query = "SELECT * FROM membership WHERE club_id = %s AND student_id = %s"
